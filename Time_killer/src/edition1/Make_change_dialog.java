...
public class Make_change_dialog extends JDialog implements ActionListener{
	private static final long serialVersionUID = -95961339925841831L;
	...
	Make_change_dialog(JFrame f,String s){
		//general setting
		super(f, s, true);
		this.title = s;
		...
		if (e.getSource() == button_ok) {
			try {
				if (check()) {
					...
					Change.change_this(
							Time_killer_ui.text_name.getText(), 
							student.toString(),
							Time_killer_ui.text_core.getText());
					Time_killer_ui.textarea.setText("已修改\n"+student.print_toString());
	...
	public void set_ts(String data) {
		boolean selected;
		for (int i = 0; i < 5; ++i) {
			for (int j = 0; j < 7; ++j) {
				...
				check_box_group[i][j].setSelected(selected);
				ts[i][j] = selected;
			}
		}
	}
	...
	public void get_all() {
		String information = "";
		try {
			information = Change.exist(Time_killer_ui.text_name.getText(), Time_killer_ui.text_core.getText());
		} catch (IOException e) {
			e.printStackTrace();
		}
		if (!information.equals("")) {
			...
			department = splited[1];
			comBox_department.setSelectedItem(department);
			student_number = splited[2];
			text_sno.setText(student_number);
			student_college = splited[3];
			text_college.setText(student_college);
			student_class = splited[4];
			text_class.setText(student_class);
			rota_day = splited[5];
			comBox_day.setSelectedItem(rota_day);
			rota_class = splited[6];
			comBox_class.setSelectedItem(rota_class);
			phone_number = splited[7];
			text_phonenum.setText(phone_number);
			birthday = splited[8];
