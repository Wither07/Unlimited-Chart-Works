...
public class Time_killer_ui extends JFrame implements ActionListener, MouseListener{
	/**
	 * 
	 */
	...
	public Time_killer_ui() {
		...
		//File chooser
		Time_killer_ui.fchooser = new JFileChooser(
				new File(FileSystemView.getFileSystemView().getHomeDirectory().getPath(),""));
		Time_killer_ui.fchooser.setFileSelectionMode(JFileChooser.FILES_ONLY);
		Time_killer_ui.fchooser.setFileFilter(new ExtensionFileFilter("txt", "文本文件(.txt)"));
        ...
        l_downer.setLayout(new GridLayout(1, 1, 0, 0));
      	...
      	l_downer.add(new JScrollPane(Time_killer_ui.textarea));
		...
		table = new JTable(Time_killer_ui.tablemodel);
	...
	public void mouseClicked(MouseEvent arg0) { 
		Time_killer_ui.quick_quit_flag = true;
	}
	public void mouseEntered(MouseEvent arg0) { 
		if(Time_killer_ui.quick_quit_flag) {
			Time_killer_ui.quick_quit_counter++;
			if(Time_killer_ui.quick_quit_counter == 3)
			{
				this.setExtendedState(JFrame.ICONIFIED);
				Time_killer_ui.quick_quit_counter = 0;
				Time_killer_ui.quick_quit_flag = false;
			}
		}
	}
	...
	public void actionPerformed(ActionEvent e) {
		//menu bar
		if (e.getSource().equals(Time_killer_ui.help)) {
		...
		if (e.getSource().equals(Time_killer_ui.info)) {
			JOptionPane.showMessageDialog(this,
					   "致山东大学威海公寓管理中心。\n"
					+ "                --by Wither  \n"
		...
		if (e.getSource().equals(Time_killer_ui.quit)) {
		...
		if (e.getSource().equals(Time_killer_ui.item5)) {
			String whosthere = "";
			try {
			...
			Time_killer_ui.textarea.setText("已展开钥匙:\n" + whosthere);
		}
		//tool bar
		if ((e.getSource().equals(Time_killer_ui.button_go))||
			 (e.getSource().equals(Time_killer_ui.text_path))){
			String command = Time_killer_ui.text_path.getText();
		...
		//left part
		if (e.getSource().equals(Time_killer_ui.button_change)) {
			Time_killer_ui.textarea.setText("修改中...");
			if (Time_killer_ui.text_name.getText().equals("")) {
				Time_killer_ui.textarea.setText("请在信息条目中输入\n要修改的人的姓名...");
			}
			else {
				try {
					if (!Change.exist(Time_killer_ui.text_name.getText(), text_core.getText()).equals("")) {
						Make_change_dialog dialog =new Make_change_dialog(this, "修改");
						dialog.setModal(false);
					}
					else {
						Time_killer_ui.textarea.setText("没有找到对象...");
					}
				} catch (IOException e1) {
					e1.printStackTrace();
				}
		...
		if ((e.getSource().equals(Time_killer_ui.button_add))||
			 (e.getSource().equals(Time_killer_ui.item1))) {
			Time_killer_ui.textarea.setText("编辑中...");
			Add_new_dialog dialog =new Add_new_dialog(this, "添加");
			dialog.setModal(false);
		}
		if (e.getSource().equals(Time_killer_ui.button_delete)) {
			boolean exist = false;
			boolean deleted = false;
			Time_killer_ui.textarea.setText("即将执行删除操作,请谨慎选择...");
			try {
				exist = Delete.exist(Time_killer_ui.text_name.getText(), text_core.getText());
			...
			if (Time_killer_ui.text_name.getText().equals("")) {
				...
				if (exist) {
					...
					if (response == 0) {
						try {
							deleted = Delete.delete_this(Time_killer_ui.text_name.getText(), text_core.getText());
						} catch (IOException e1) {
							e1.printStackTrace();
						} finally {
							if (deleted) {
								Time_killer_ui.textarea.setText("已删除");
							}
							else {
								Time_killer_ui.textarea.setText("删除失败!");
							}
						}
					}
					else {
						Time_killer_ui.textarea.setText("操作已取消");
					}
				}
				else {
					Time_killer_ui.textarea.setText("没有找到对象...");
		...
		if ((e.getSource().equals(Time_killer_ui.button_find)) ||
	     (e.getSource().equals(Time_killer_ui.text_name))) {
			...
			String filePath = Time_killer_ui.text_core.getText();
			try {
				if (!name.equals("")&&Change.exist(name, filePath).equals("")) {
					...
					try {
						Time_killer_ui.result = Find.give_a_result(name, department, day, time, filePath);
					} catch (IOException e1) {
						e1.printStackTrace();
					}
					Time_killer_ui.r1.removeAll();
					Time_killer_ui.r1.setVisible(false);
					Time_killer_ui.tablemodel = new DefaultTableModel(result, titles_day);
					Time_killer_ui.table = new JTable(Time_killer_ui.tablemodel);
					Time_killer_ui.table.setRowHeight(98);
					Time_killer_ui.r1.add(new JScrollPane(table));
					if ((!day.equals("-不限-"))&&(!time.equals("-不限-"))) {
						Time_killer_ui.tablemodel.setRowCount(5);
					}
					Time_killer_ui.r1.setVisible(true);
					Time_killer_ui.textarea.setText("已完成查找");
				}
			} catch (IOException e1) {
				e1.printStackTrace();
			}
		}
		if ((e.getSource().equals(Time_killer_ui.button_info))||
		     (e.getSource().equals(Time_killer_ui.item2)))
		    {
			if (Time_killer_ui.text_name.getText().equals("")) {
				Time_killer_ui.textarea.setText("请在信息条目中输入\n要显示的人的姓名...");
			}
				...
				String res = "";
				try {
					res = Change.exist(Time_killer_ui.text_name.getText(), Time_killer_ui.text_core.getText());
				} catch (IOException e1) {
					e1.printStackTrace();
				}
				if(!res.equals("")) {
					Time_killer_ui.textarea.setText(Student.print_toString(res));
				}
				else {
					Time_killer_ui.textarea.setText("没有找到对象...");
				}
		...
		if ((e.getSource().equals(Time_killer_ui.button_switch)) ||
		     (e.getSource().equals(Time_killer_ui.item3))) {
			String solution = "";
			try {
				solution = Switch.give_solution(Time_killer_ui.text_name.getText(), Time_killer_ui.text_core.getText());
			} catch (IOException e1) {
				e1.printStackTrace();
			}
			if(!solution.equals("")) {
				Time_killer_ui.textarea.setText("换班方案:\n"+solution);
			}
			else {
				Time_killer_ui.textarea.setText("没有找到对象...");
			}
		}
		if ((e.getSource().equals(Time_killer_ui.button_create))||
		     (e.getSource().equals(Time_killer_ui.item4))) {
			Time_killer_ui.textarea.setText("请选择需要创建的表格...\n"
					+ "将自动在桌面上生成文件夹\n"
		...
		//right part
		if (e.getSource().equals(Time_killer_ui.button_core)) {
			if (Time_killer_ui.fchooser.showOpenDialog(Time_killer_ui.fchooser)==0)
	        {
	        	Time_killer_ui.file = Time_killer_ui.fchooser.getSelectedFile();
	        	Time_killer_ui.text_core.setText(Time_killer_ui.file.getAbsolutePath());
	        	if (file.exists()) {
	        		Enable_some();
	        		unlockable = true;
	        		Time_killer_ui.textarea.setText("正在解除封印...\n封印解除!\n");
	        	}
	        }
		}
